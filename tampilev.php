...
if(isset($_SESSION['masuk'])) {  
   ...
   if ($_SESSION['level'] == "admin") {
    ...
    $user = '';
        ...
        $id=$_GET['id'];
        $query_mysql = mysql_query("SELECT* FROM tb_event WHERE id_ev='$id'", $conn);
        $nomor=1;
        $data=mysql_fetch_array($query_mysql);
        $suka = $data['suka'];
        $lihat = $data['lihat'] +1;
        mysql_query("UPDATE tb_event SET lihat='$lihat' WHERE id_ev='$id'", $conn);
        ...
        if (isset($_POST['batalsuka'])) {
          ...
          $simpann="DELETE FROM tbl_like WHERE id_ev='$id_post' AND id_user='$id_user'";
        ...
        if (isset($_POST['suka'])) {
          ...
          $input=mysql_query($simpann);
          $suka++;
        mysql_query("UPDATE tbl_event SET suka='$suka' WHERE id_ev='$id'", $conn);
        }
...
<form action="login1.php">
      ...
      <p class="lead">Postingan ini di post oleh :</p>
        <p><b><?= $data['id_author']; ?><br><img src="kalender.jpg" width="25" height="25"></b>  <small class="text-monospace"><?= $data['tgl_post']; ?></small></p>
      <hr class="my-7">
    </form>
...
<div class="container">
  ...
  <img src="like.png" width="45" height="35"><?=$data['suka']?></div>
      ...
      <h1><?= $data['judul_post'] ?></h1>
      <div class="container"><p><?= $data['isi_post'] ?></p></div>
    </div>
  </div>
</div>
</section>
...
<?php while ($datak = mysql_fetch_array($tampilk)) { ?>
<div class="media text-dark">
    <div class="media-body">
      <p class="mt-0"><b><?= $datak['user_komen']; ?></b> <small class="text-monospace"><?= $datak['waktu']?></small></p>
      <?= $datak['isi_komen'] ?>
    </div>
</div>
<?php } ?>
  <form action="" method="POST">
    <div class="form-group col-lg-12">
        <textarea name="komentar" class="form-control" placeholder="Tulis Komentar..." rows="5" name="isi" value=""></textarea>
        <div class="nav justify-content-end bg-light">      
          <ul class="nav">
          <li class="nav-item">
            <?php if (!isset($masuk)) { ?>
              <button class="btn btn-light" name="suka" href="#" disabled="True"><img src="like.png" height="25" width="25" > Suka</button>
            <?php }elseif (isset($liked)) { ?>
              <button class="btn btn-info" name="batalsuka" href="#"><img src="like.png" height="25" width="25" >Batal Suka</button>
            <?php }else{?>
              <button class="btn btn-light" name="suka" href="#"><img src="like.png" height="25" width="25" > Suka</button>
            <?php } ?>
            
          </li>
          <li class="nav-item">
            <?php if (isset($admin) || isset($user )) { ?>
            <button name="tmbkom" class="btn btn-dark ml-2"><img src="komen.png" height="25" width="25">Posting Komentar</button>
          <?php }else{ ?>
            <button disabled="true" name="tmbkom" class="btn btn-dark ml-2"><img src="komen.png" height="25" width="25">Posting Komentar</button>
          <?php } ?>
          </li>
        </ul>
        </div>
