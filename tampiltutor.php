...
if(isset($_SESSION['masuk'])) {  
   ...
   if ($_SESSION['level'] == "admin") {
    ...
    $user = '';
        ...
        $id=$_GET['id'];
        $query_mysql = mysql_query("SELECT* FROM tbl_tutor WHERE id_tutor='$id'", $conn);
        $nomor=1;
        $data=mysql_fetch_array($query_mysql);
        $suka = $data['suka'];
        $lihat = $data['lihat'] +1;
        mysql_query("UPDATE tbl_tutor SET lihat='$lihat' WHERE id_tutor='$id'", $conn);
        ...
        if (isset($_POST['batalsuka'])) {
          ...
          $simpann="DELETE FROM tbl_like WHERE id_tutor='$id_post' AND id_user='$id_user'";
...
<form action="login1.php">
      ...
      <p class="lead">Postingan ini di post oleh :</p>
        <p><b><?= $data['id_author']; ?><br><img src="kalender.jpg" width="25" height="25"></b>  <small class="text-monospace"><?= $data['tgl_tutor']; ?></small></p>
      <hr class="my-7">
    </form>
...
<div class="container">
  ...
  <img src="mata.png" width="35" height="35"><?=$data['lihat']?></div>
    <div class="float-right">
  <img src="like.png" width="35" height="35"><?=$data['suka']?></div>

      <img class="img-thumbnail" src="<?= $data['gambar'] ?>" width="400" height="400">
      <h1><?= $data['judul_tutor'] ?></h1>
      <div class="container"><p></p><?= $data['isi_tutor'] ?></p></div>
    </div>
  </div>
</div>
</section>
