...
$tampil1=mysql_query("SELECT * FROM tb_tutor",$conn);
  if (isset($_GET['tmbcari'])) {
    $txtcari = $_GET['cari'];
      $tampil1 = mysql_query("SELECT * FROM tb_tutor where judul_tutor LIKE '%$txtcari%'");
      }
    }
?>
<?php
...
<nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-dark">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
   ...
   <a class="nav-link" href="tambahevent.php">Tambah event</a></li>
      ...
      <li class="nav-item">
      <?php if (isset($admin)) { ?>
   <a class="nav-link" href="tambahtutor.php">Tambah tutor</a></li>
       <?php } ?>
                <li class="nav-item">
	...
	<div class="col-3">
		<div class="card text-right" style="width: 18rem;">
		  ...
		  <div class="card-body">
		    ...
		    <a href="tampil.php?id=<?= $data['id_post'] ?>" class="btn btn-info">baca selengkapnya</a>		
		    <?php if (isset($admin)) { ?>
		  	<a href="formedit.php?id=<?=$data['id_post'] ?>" class="btn btn-secondary">sunting</a>	 
			<?php } ?>
			<?php if (isset($admin)) { ?>
			<center><a href="hapus.php?id=<?=$data['id_post'] ?>" class=" btn btn-danger mt-2">hapus</a></center>
			<?php } ?>
		  </div>
		</div>
	</div>
